...
public struct DMRCompliance {

    /// 是否符合某种的规则
    /// - Parameters:
    ///   - match: 需要判断的
    ///   - regex: 正则表达式
    public static func compliance(_ match: String,_ regex: String) -> Bool {
        let predicate = NSPredicate(format: "SELF MATCHES %@", regex)
        return predicate.evaluate(with: match)
    }
}

extension DMRCompliance {
    
       /// 是否遵循iP的规范
       /// - Parameter ip: ipv4
       public static func compliance(ip: String) -> Bool {
       ...
       public static func compliance(port: String) -> Bool {
